...
function calculate_price_ecotasa($product, $taxonomy) {
    ...
    //$IVA_price = $base_price * 0.21;
    //$result = $base_price + $IVA_price;
    //return number_format($result, 2);


    switch ($taxonomy) {
        case "vehiculo":
            $ecotasa_vehiculo = 1.98;
            $IVA_price = $base_price * 0.21;
            $result = $base_price + $IVA_price + $ecotasa_vehiculo;
            $price = number_format($result, 2);
            break;
        case "carga":
            $ecotasa_carga = 1.98;
            $IVA_price = $base_price * 0.21;
            $result = $base_price + $IVA_price + $ecotasa_carga;
            $price = number_format($result, 2);
            break;
        case "moto":
            $ecotasa_moto = 1.98;
            $IVA_price = $base_price * 0.21;
            $result = $base_price + $IVA_price + $ecotasa_moto;
            $price = number_format($result, 2);
            break;
        case "comercial":
            $ecotasa_comercial = 1.98;
            $IVA_price = $base_price * 0.21;
            $result = $base_price + $IVA_price + $ecotasa_comercial;
            $price = number_format($result, 2);
            break;
        case "todoterreno":
            $ecotasa_todoterreno = 1.98;
            $IVA_price = $base_price * 0.21;
            $result = $base_price + $IVA_price + $ecotasa_todoterreno;
            $price = number_format($result, 2);
            break;
        case "camion":
            $ecotasa_camion = 7.11;
            $IVA_price = $base_price * 0.21;
            $result = $base_price + $IVA_price + $ecotasa_camion;
            $price = number_format($result, 2);
            break;
        case "turismo":
            $ecotasa_turismo = 1.98;
            $IVA_price = $base_price * 0.21;
            $result = $base_price + $IVA_price + $ecotasa_turismo;
            $price = number_format($result, 2);
            break;
    }
...
function inject_custom_html_in_product_description() {
    ...
    // Definir el slug de la taxonomía personalizada
    $taxonomy_slug = 'neumatico-vehiculo';

    // Obtener los términos de la taxonomía personalizada asociados al producto
    $product_terms = wp_get_post_terms($product->get_id(), $taxonomy_slug);

    // Si el producto no tiene términos en la taxonomía, no hacer nada
